  ...
  /* USER CODE BEGIN mainTask */
   ...
   for(;;)
   {
      ...
      if(call == 1){
         ...
         settings.offsetMax = Sensor1.getOffsetMax();
         settings.offsetMin = Sensor1.getOffsetMin();
         Flash_Write(settings, StartSettingsAddres);
      }
      if(call == 2){
        call = 0;
        Sensor2.Call(&adc_buffer[1]);
        Flash_Write(settings, StartSettingsAddres);
      }
      if(call == 3){
        call = 0;
        Sensor3.Call(&adc_buffer[2]);
        Flash_Write(settings, StartSettingsAddres);
      }
     ...
     case MB_REG_WRITE:
      {	
         ...
         switch (usAddress)
         {
           case 0: //  Stop/Start
            {	
              uint16_t temp = ((*(pucRegBuffer)<<8) | (*(pucRegBuffer+1)));
              if((temp > 1) & (temp < 100)){
                Sensor1.timeCall = temp * 1000;
                call = 1;
              }
